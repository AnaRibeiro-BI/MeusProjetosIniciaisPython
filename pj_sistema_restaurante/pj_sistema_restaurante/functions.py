...
def editar_produto(nome_antigo, nome_novo, valor):
    try:    
        conexao = bd.conectar_db()
        cursor =  conexao.cursor()
        sql_query = "UPDATE produtos SET nome = ?, valor = ? WHERE nome = ?"
        parametros = (nome_novo, valor, nome_antigo)

        cursor.execute(sql_query, parametros)
        conexao.commit()

        print(f"Produto '{nome_antigo}' editado com sucesso!")
        return True
    except Exception as e:
        print(f"Erro ao editar itens: {e}")
        if conexao:
            conexao.rollback()
        return False
    finally:
...
editar_produto("Amstel", "Amstel Lata", 3.69)